...
class Q:
    def __init__(self,scale=7,explore = 0.01,is_load = True):
    ...
    def run(self, now, dead, action):
        ...
        if self.prev is None :
            ...
            self.Q[self.prev[0],self.prev[1], self.prev[2]] += self.lr * \
                (reward + self.gamma * (np.max(self.Q[state[0], state[1]])) -
                    self.Q[self.prev[0], self.prev[1], self.prev[2]])
            if reward < 0 :
                self.prev = None
